...
/* ============================================================
   MAIN SERVICE FUNCTION — decodeAndStore(signature)

   This is the extracted business logic from the route handler.
   Can be called from webhook, route, or any other entry point.

   Returns the decoded swap object on success.
   Throws an Error on failure (invalid tx, MEV, no DEX, etc.)
============================================================ */
...
async function decodeAndStore(signature) {
    ...
    const rpc = await axios.post(
        ...
        { timeout: 15000 } // 15 sec safety — prevents hanging webhook threads
    );
    ...
    if (!message) {
        throw new Error("Could not parse transaction message");
    }
    ...
    if (isMev) {
        throw new Error("MEV/arbitrage transaction — no net user swap to decode");
    }

    if (!changes.size) {
        throw new Error("No balance changes detected");
    }
    ...
    if (!negatives.length || !positives.length) {
        throw new Error("Invalid swap structure — could not identify token_in / token_out");
    }
    ...
    if (!dexResult) {
        throw new Error("Unsupported DEX — no known DEX program found in transaction");
    }
